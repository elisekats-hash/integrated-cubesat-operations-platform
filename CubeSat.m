...
if nargin > 0 && isfield(correction, 'packet_type')
    if strcmp(correction.packet_type, 'correction') || correction.packet_type == "correction"
        ...
        % MATLAB converts JSON spaces to underscores (e.g., 'reaction wheels' -> 'reaction_wheels')
...
% FIXED: Changed from .time to .timestamp to perfectly match Elise's Python dictionary
telemetry.timestamp = position.time;
